...
From Question 5, we know that the maximum likelihood estimator
\(\hat\theta_{ML} = \bar X_n\) satisfies
\[
\sqrt{n}(\hat\theta_{ML}-\theta)
\xrightarrow{d}
\mathcal N(0,\theta)
\]

Therefore, by standardization,
\[
\frac{\hat\theta_{ML}-\theta}{\sqrt{\theta/n}}
\xrightarrow{d}
\mathcal N(0,1)
\]
...
We then obtain the following CI for \(\theta\)
\[
\theta \in
\left[
\hat\theta_{ML}
- z_{1-\alpha/2}\sqrt{\frac{\theta}{n}},
\;
\hat\theta_{ML}
+ z_{1-\alpha/2}\sqrt{\frac{\theta}{n}}
\right].
...
Since \(\theta\) is unknown, we replace it by 
\(\hat\theta_{ML}\).  

Therefore, an asymptotic confidence interval of level \(1-\alpha\) for \(\theta\) is:
\[{
\left[
\hat\theta_{ML}
- z_{1-\alpha/2}\sqrt{\frac{\hat\theta_{ML}}{n}},
\;
\hat\theta_{ML}
+ z_{1-\alpha/2}\sqrt{\frac{\hat\theta_{ML}}{n}}
\right]}
...
For a Poisson distribution with parameter \(\theta\), we have
\[
\mathbb E[X]=\theta,
\qquad
\mathrm{Var}(X)=\theta.
...
The method of moments equates empirical moments with their theoretical parameters, assuming empirical moments will eventually get close to the theoretical parameters

For the first moment, we define the estimator
\[
\hat\theta_1 = \bar X_n.
\]

For the second moment, we define the estimator
\[
\hat\theta_2
=
...
The estimator \(\hat\theta_1\) is unbiased since
\[
\mathbb E[\hat\theta_1]=\theta.
\]
Moreover, \(\hat\theta_1\) is the same as the maximum likelihood estimator and is consistent.

### Q9

From Question 4, we know that the maximum likelihood estimator is
\[
\hat\theta_{ML} = \bar X_n.
\]
...
and therefore
\[
\mathrm{Bias}(\hat\theta_{ML}) = 0.
\]

Variance:
\[
\mathrm{Var}(\hat\theta_{ML})
=
\mathrm{Var}(\bar X_n)
=
...
R(\hat\theta_{ML},\theta)
=
\mathbb E[(\hat\theta_{ML}-\theta)^2]
=
\mathrm{Var}(\hat\theta_{ML})
+
\mathrm{Bias}(\hat\theta_{ML})^2
=
\frac{\theta}{n}.
\]
